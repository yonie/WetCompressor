#!/bin/bash
echo "================================================"
echo " WetCompressor VST3 Plugin - Build Script"
echo "================================================"
echo ""
...
if [ -d "WetCompressor/build" ]; then
    echo "Cleaning previous build..."
    rm -rf WetCompressor/build
fi

echo "Creating fresh build directory..."
mkdir -p WetCompressor/build
cd WetCompressor/build
...
if [ "$PLATFORM" = "macos" ]; then
    ...
    if [ -d "Release/WetCompressor.vst3" ]; then
        echo "Plugin location: WetCompressor/build/Release/WetCompressor.vst3"
        echo ""
        
        echo "Step 3: Code signing for macOS..."
        echo "================================================"
        codesign --force --deep --sign - "Release/WetCompressor.vst3"
        if [ $? -eq 0 ]; then
        ...
        if [ -f "../vst3sdk/build/bin/validator" ]; then
            ../vst3sdk/build/bin/validator "Release/WetCompressor.vst3"
            if [ $? -eq 0 ]; then
                echo ""
    ...
    if [ -f "VST3/Release/WetCompressor.vst3/Contents/x86_64-linux/WetCompressor.so" ]; then
        echo "Plugin location: WetCompressor/build/VST3/Release/WetCompressor.vst3"
        echo ""
        
        echo "Step 3: Running VST3 validator..."
        echo "================================================"
        if [ -f "bin/Release/validator" ]; then
            bin/Release/validator "VST3/Release/WetCompressor.vst3"
            if [ $? -eq 0 ]; then
                echo ""
                echo "Validation passed!"
            else
